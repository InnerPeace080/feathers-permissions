...
function processRole(context,role,rolesInfo){
  ...
  if (roleUrlInfo.limit && roleUrlInfo.limit.whiteList &&
        roleUrlInfo.limit.whiteList.idField &&  roleUrlInfo.limit.whiteList.idValue) {
    let idValue = _.get(context.params,`${roleUrlInfo.limit.whiteList.entity||'user'}.${roleUrlInfo.limit.whiteList.idField}`)
    if (!roleUrlInfo.limit.whiteList.idValue.includes(idValue)) {
      throw new Forbidden('You do not have the correct permissions.');
    }
  }
  if (roleUrlInfo.limit && roleUrlInfo.limit.blackList &&
        roleUrlInfo.limit.blackList.idField &&  roleUrlInfo.limit.blackList.idValue) {
    let idValue = _.get(context.params,`${roleUrlInfo.limit.blackList.entity||'user'}.${roleUrlInfo.limit.blackList.idField}`)
    if (roleUrlInfo.limit.blackList.idValue.includes(idValue)) {
      throw new Forbidden('You do not have the correct permissions.');
    }
  }
  ...
  if (roleUrlInfo.limit &&
    ...
    let processField = (data,fieldInfo)=>{
      ...
      if(fieldInfo.force !== undefined){
        if ( isObject(fieldInfo.force) && fieldInfo.force.idField) {
