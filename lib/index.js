...
function processRole(context, role, rolesInfo, options={}){
  ...
  const roleUrlInfo = roleInfo.permissions.find((c)=>((c.url===context.path || c.url==='all') &&
                                                      (c.method === context.method || c.method === 'all' ||
                                                        (Array.isArray(c.method) && c.method.includes(context.method))) ));
  ...
  context.params.roleApiOptions = (roleUrlInfo.limit && roleUrlInfo.limit.apiOptions) || {};
  const notAllowEmptyId = context.params.roleApiOptions.notAllowEmptyId || false;
  ...
  if (roleUrlInfo.limit && roleUrlInfo.limit.restrict) {
    roleUrlInfo.limit.restrict.forEach((restrictInfo)=>{
      ...
      if (restrictInfo.idField) {
        ...
        if(isObject(restrictInfo.idValue)){
          idValue={};
          Object.keys(restrictInfo.idValue).forEach((c)=>{
            if(c.indexOf('_$')===0){
              idValue[c.replace('_$', '$')]=restrictInfo.idValue[c];
            }else{
              idValue[c]=restrictInfo.idValue[c];
            }
      ...
      if (idValue !== undefined && restrictInfo.ownerField) {
        if(restrictInfo.ownerField === '__id__' &&
          ...
          context.id = idValue;
        }else if(restrictInfo.ownerField !== '__id__'){
  ...
  if(context.method === 'get' || context.method === 'patch' || context.method === 'remove'){
      if(notAllowEmptyId && !context.id){
        throw new Forbidden('Missing id.');
      }
  }
...
module.exports = function checkPermissions (options = {}) {
  ...
  const { entity: entityName, field, roles, preventEditAll } = options;
