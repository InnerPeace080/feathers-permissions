...
function processRole(context,role,rolesInfo){
  ...
  if (roleUrlInfo.limit && roleUrlInfo.limit.restrict) {
    const restrictInfo = roleUrlInfo.limit.restrict
    var idValue
...
module.exports = function checkPermissions (options = {}) {
  ...
  if (!Array.isArray(roles) && typeof roles !== 'function') {
    throw new Error(`'roles' option for feathers-permissions context must be an array or a function`);
  }

  return function (context) {
    return Promise.resolve(typeof roles === 'function' ? roles(context) : roles).then(currentRoles => {
      if (context.type !== 'before') {
        return Promise.reject(new Error(`The feathers-permissions context should only be used as a 'before' context.`));
      }
      debug('Running checkPermissions context with options:', options);
      const entity = context.params[entityName];
      const rolesInfo = context.rolesInfo

      if (!entity) {
        debug(`context.params.${entityName} does not exist. If you were expecting it to be defined check your context order and your idField options in your auth config.`);
      ...
      if (requiredPermissions) {
        ...
        permissions.some(()=>{
          context = processRole(context,permissions,rolesInfo)
          return context
        })
      }else{
